...
import { 
  ...
  export type Action =
    ...
    | { type: ActionType.APPLY_TEXT_FORMAT; payload: { operation: TextFormatting } }
...
export type ActionHandler = {
    [K in TextFormatting | keyof typeof Alignment | ToolbarTableAction as 
        | `onClick${K}` 
        | `handle${K}`]: () => void;
};
...
export type BaseClickHandlers = {
    [K in keyof typeof Alignment | TextFormatting | ToolbarTableAction as 
        | `onClick${K}` 
        | `handle${K}`]: () => void;
};
